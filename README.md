...
## 🚀 快速开始
### 命令行使用 (CLI)
本项目提供了一个简单的命令行入口 `main.py`，可以快速生成 MIDI 文件。
**基本用法：**
```bash
python main.py --preset pop --output output.mid
```
**参数说明：**
-   `--preset`: 预设名称（默认: `pop`）。可选值包括 `pop`, `jazz`, `rock`。
-   `--output`: 输出的 MIDI 文件路径（必填）。
-   `--bars`: 生成的小节数量（可选，覆盖预设中的设置）。
**示例：**
生成一段 32 小节的爵士音乐：
```bash
python main.py --preset jazz --bars 32 --output my_jazz_song.mid
```
### 预设配置
预设文件位于 `presets/` 目录下。你可以通过修改 JSON 文件来定制音乐风格。
**示例：`presets/pop.json`**
```json
{
  "tempo": 120,
