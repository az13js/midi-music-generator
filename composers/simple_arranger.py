...
class SimpleArranger:
    def __init__(self, tempo: int, key: Key):
        ...
        self.meta_track.append(MetaMessage('set_tempo', microseconds=microseconds_per_beat))
        
        # 2. 设置拍号 (Time Signature) - 默认 4/4
        self.meta_track.append(MetaMessage('time_signature', numerator=4, denominator=4, 
                                           clocks_per_click=24, notated_32nd_notes_per_beat=8))
        
        # 3. 设置调号 (Key Signature)
        # mido 使用 'sharps' (正数为升号，负数为降号) 表示调号
        # 这里做一个简单的映射，覆盖基础大调和小调
        key_signature_map = {
        ...
        # 获取调号对应的升降号数量，默认为 C大调 (0)
        sharps = key_signature_map.get(key.value, 0)
        # MIDI key format: (sf, mi), mi=0 for major, 1 for minor
        is_minor = key.value.endswith('m')
        self.meta_track.append(MetaMessage('key_signature', key=sharps if not is_minor else sharps, mode=1 if is_minor else 0))
